...
public interface CoecMicroTaskingDao {
  ...
  @Query("SELECT * FROM coec_microtasking WHERE (at_latitude < :north_lat AND at_latitude > :south_lat) AND (at_longitude > :west_lng AND at_longitude < :east_lng) AND NOT marked_complete ORDER BY begins")
  LiveData<List<CoecMicroTasking>> get_incomplete_for_area(double north_lat, double west_lng, double south_lat, double east_lng);

  @Query("SELECT * FROM coec_microtasking WHERE (at_latitude < :north_lat AND at_latitude > :south_lat) AND (at_longitude > :west_lng AND at_longitude < :east_lng) AND marked_complete ORDER BY begins")
